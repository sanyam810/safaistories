...
const AdminHighlightCard = ({ key, item }) => {
  ...
  const handleDelete = async () => {
    try{
      const res= await fetch(`http://localhost:3000/api/adminHighlight/${item.slug}`,{
        method: 'DELETE',
    });
    if(res.ok){
  ...
  const handleSetFeatured = async () => {
    try {
      ...
      if (res.ok) {
        toast.success('Post set as featured successfully');
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      }
      if (!res.ok) {
        toast.error('Error setting post as featured');
      }
    } catch (err) {
      console.log('Set as featured error', err);
    }
  }
    ...
    return (
      <div className="bg-white shadow-lg rounded-md p-6 mb-8" style={{marginBottom:'12px'}}>
        ...
        <p className="text-gray-600 mb-4">{item.summary.substring(3, 150)}...</p>
        ...
        <Button variant="secondary" onClick={handleSetFeatured}>Set as Featured</Button>
      </div>
    );
  };
